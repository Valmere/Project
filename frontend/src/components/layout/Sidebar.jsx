  ...
  return (
    ...
    >
      ...
      <div className="flex-shrink-0 px-5 pt-5 pb-4 flex items-center gap-3">
        ...
        <button
          onClick={onClose}
          className="lg:hidden p-1 text-white/40 hover:text-white transition-colors flex-shrink-0 ml-auto"
          aria-label="Fermer"
        >
          {Ic.close}
        </button>
            ...
            >
              {({ isActive }) => (
                <>
                  {isActive && (
                    <span
                      className="absolute left-0 inset-y-[6px] w-0.5 rounded-r"
                      style={{ background: 'var(--color-secondary)' }}
                    />
                  )}
                  <span className="flex-shrink-0 w-[18px]">{Ic[item.icon]}</span>
