...
export default function ChartOfAccountsPage() {
  ...
  const handleDelete = async (a) => {
    if (!confirm(t('coa.confirm_delete', { code: a.code, name: a.name }))) return
    try {
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-xl font-bold text-slate-800">{t('coa.title')}</h2>
          <p className="text-sm text-slate-500 mt-1">{t('coa.subtitle')}</p>
        </div>
      ...
      {showNew && (
        <form onSubmit={submit} className="bg-white rounded-xl shadow-sm p-5 grid grid-cols-6 gap-3">
